...
class LifeLone_MNIST(object):
    ...
    def Create_Component(self,index):
        if np.shape(self.lossArr)[0] == 0:
            ...
            # set 5000 if gpu has more memories
            # n_samples = 1000
            z_test = qzx_test.sample(2)

            sumZ_test = z_test * b0[0]
            for i in range(np.shape(self.latentZArr)[0] - 1):
                sumZ_test = sumZ_test + self.evalLatentZArr[i] * b0[i + 1]
            ...
            sumZ_genertor_test = latentX1_test * decoderWeight[0]
            for i in range(np.shape(self.latentXArr)[0] - 1):
                sumZ_genertor_test = sumZ_genertor_test + self.evalLatentXArr[i] * decoderWeight[i + 1]
    ...
    def SelectSample_InMemory(self):
        ...
        count = int(np.shape(self.DynamicMmeory)[0]/self.batch_size)
        remainCount = np.shape(self.DynamicMmeory)[0] - self.batch_size * count
        remainSamples = self.DynamicMmeory[self.batch_size * count:np.shape(self.DynamicMmeory)[0]]
        remainSamples = np.concatenate((remainSamples,self.DynamicMmeory[0:self.batch_size-np.shape(remainSamples)[0]]),axis=0)

        totalLoss = np.zeros((np.shape(self.DynamicMmeory))[0])
        for t in range(np.shape(self.allLossArr)[0]):
            lossArr = []
            evaluatedLoss = self.allLossArr[t]
            for i in range(count):
                batch = self.DynamicMmeory[i*self.batch_size:(i+1)*self.batch_size]
                loss1 = self.sess.run(evaluatedLoss, feed_dict={self.input_x: batch})
                if np.shape(lossArr)[0] == 0:
                    lossArr = loss1
                else:
                    lossArr = np.concatenate((lossArr,loss1),axis=0)
            loss1 = self.sess.run(evaluatedLoss, feed_dict={self.input_x: remainSamples})
            loss1 = np.array(loss1)
            lossArr = np.concatenate((lossArr,loss1[0:remainCount]),axis=0)
            lossArr = np.array(lossArr)
            totalLoss = totalLoss + lossArr

        totalLoss = np.array(totalLoss)
        index = np.argsort(totalLoss)
        index = index[0:self.maxMmeorySize]
                ...
                for epoch in range(epochs):
                    ...
                    if np.shape(self.DynamicMmeory)[0] > self.maxMmeorySize:
                        ...
                        if diff > threshold:
                            ...
                            #Sample selection in the memory
                            '''
