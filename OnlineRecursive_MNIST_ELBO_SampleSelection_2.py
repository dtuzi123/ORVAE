...
class LifeLone_MNIST(object):
    ...
    def SelectSample_InMemory(self):
        evaluatedLoss = self.EvaluationLossArr[np.shape(self.EvaluationLossArr)[0]-1]
        lossArr = []
        for i in range(np.shape(self.DynamicMmeory)[0]):
            a1 = self.DynamicMmeory[i]
            a1 = np.reshape(a1,(-1,28*28))
            loss1 = self.sess.run(evaluatedLoss,feed_dict={self.input_test:a1})
            lossArr.append(loss1)
