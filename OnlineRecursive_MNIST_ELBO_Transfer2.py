...
class LifeLone_MNIST(object):
    ...
    def Train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for index in range(totalCount - 1):
                batch = self.totalSet[index * self.batch_size : (index + 1) * self.batch_size]
                batch_last = self.totalSet[(index+1) * self.batch_size : (index + 2) * self.batch_size]

                if np.shape(self.auxilaryData)[0] == 0:
                    self.auxilaryData = batch
                    self.auxilaryData = np.concatenate((self.auxilaryData,batch_last),axis=0)
                else:
                    self.auxilaryData = np.concatenate((self.auxilaryData,batch_last),axis=0)
            ...
            f = open("results/ORVAE_forwardTf_currentELBO" + str(0) + ".txt", "w", encoding="utf-8")
            ...
            f = open("results/ORVAE_forwardTf_diff" + str(0) + ".txt", "w", encoding="utf-8")
            for i in range(np.shape(lossArr1)[0]):
                f.writelines(lossArr1[i])
                f.writelines('\n')
            f.flush()
